"""
Phase 2: Web Scraper + ML Anomaly Detection
============================================
Location: multiagent/core/phases/phase2.py

Combines:
  - WebScraper           → scrapes QS/THE rankings from web, caches results
  - Phase2AnomalyDetector→ Isolation Forest validates scraped data before saving
  - RankingExtractor     → NLP regex extracts rankings from raw HTML text
  - score_scraped_record → quality scorer for any scraped record

Why ML here:
  Without ML: save whatever was scraped (broken HTML? wrong page? site changed?)
  With ML:    validate every scraped value before it touches the database
              flag suspicious changes (rank 32 → 320 = something went wrong)
              quality score tells you how trustworthy the scraped data is
...
Usage:
    ...
    # Scrape rankings
    results = p2.scraper.scrape_qs_rankings()
    results = p2.scraper.scrape_the_rankings()

    # ML: validate before saving
    check = p2.ml.check_ranking("uk_manchester", new_qs=35, new_the=56)
    batch = p2.ml.batch_check_rankings(scraped_dict)

    # Extract from raw text
    data  = p2.ml.extract_from_text(html_text, currency="GBP")

    # Quality score
    from core.phases.phase2 import score_scraped_record
    quality = score_scraped_record(record)
"""
