...
function ALevelManualForm({stream,onSubmit,onBack}){
  ...
  return(
    <div>
      ...
      {err&&<Alert type="error">{err}</Alert>}
      <div className="btn-row"><button className="btn btn-ghost" onClick={onBack}>← Back</button><button className="btn btn-primary" onClick={submit}>Save Document →</button></div>
    </div>
  );
}
  ...
  return(
    <div className="fade-up">
      ...
      <div className="panel">
          ...
          <div>
            ...
            {aiResult&&(
              <div style={{marginTop:"1.25rem"}}>
                ...
                {Object.keys(uploadedDocs).length===0&&(
                  <Alert type="info">At least one extracted document is needed to enable the eligibility check.</Alert>
                )}
                <div className="btn-row" style={{marginTop:"1.5rem"}}>
                  <button className="btn btn-ghost" onClick={onBack}>← Profile</button>
                  <button className="btn btn-primary" onClick={proceed} disabled={Object.keys(uploadedDocs).length===0}
                    title={Object.keys(uploadedDocs).length===0?"Upload and extract at least one document to enable":""}
        ...
        {tab==="manual"&&(
          <div>
            <div className="slabel" style={{marginTop:0}}>Select Document Type to Enter</div>
            <div className="doctype-grid" style={{marginBottom:"1.25rem"}}>
              {DOC_TYPE_LIST.map((dt)=>{
                const done=Boolean(uploadedDocs[dt.id]);
                return(
                  <div
                    key={dt.id}
            ...
            {Object.keys(uploadedDocs).length===0&&(
              <Alert type="info">Enter and save at least one document above to enable the eligibility check.</Alert>
            )}
            <div className="btn-row" style={{marginTop:"1.25rem"}}>
              <button className="btn btn-ghost" onClick={onBack}>← Profile</button>
              <button className="btn btn-primary" onClick={proceed} disabled={Object.keys(uploadedDocs).length===0}
                title={Object.keys(uploadedDocs).length===0?"Add at least one document to enable":""}
              >Check Eligibility →</button>
            </div>
          </div>
        )}
